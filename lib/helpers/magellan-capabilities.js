...
/**
 * Paste-ready dump of every device an account announced over Magellan.
 *
 * A device no driver claims never reaches CozyTouchDevice, so its capability
 * list — the one thing adding support for it needs — used to be visible only in
 * a diagnostic report taken during a *successful* pairing. The setup view
 * carries that list for every device (see CozyTouchAPI.getSetupCapabilities),
 * so the settings page can hand it over without pairing anything.
 *
 * Both IDs are printed because capability IDs are per product, not per model
 * family: see WATER_HEATER_CAP_IDS_BY_PRODUCT and CLIMATE_CAP_IDS_BY_PRODUCT.
 *
 * @param {object[]} devices - devices as the setup view returns them
 * @param {function(number): string} typeOf - CozyTouchAPI.getDeviceType
 * @param {string} [appVersion]
 */
function describeAccount(devices, typeOf, appVersion) {
  const list = asList(devices);
  const header = `Cozytouch (Magellan) — ${list.length} device(s)`;
  const lines = [appVersion ? `${header} — app ${appVersion}` : header];

  for (const dev of list) {
    const type = typeof typeOf === 'function' ? typeOf(dev.modelId) : '?';
    lines.push('');
    lines.push(`${dev.name || '(unnamed)'} | modelId ${dev.modelId} | productId ${dev.productId} | type ${type}`);
    lines.push(`  ${describeCapabilities(dev.capabilities)}`);
  }
  return lines.join('\n');
}
