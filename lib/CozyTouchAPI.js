...
class CozyTouchAPI {
  ...
  async getSetup() {
    ...
    // Debug: dump first device structure and capabilities metadata
    if (this._devices.length > 0) {
      this._log(`First device keys: ${JSON.stringify(Object.keys(this._devices[0]))}`);
      const devCaps = this._devices[0].capabilities || [];
      if (devCaps.length > 0) {
        this._log(`First device has ${devCaps.length} capabilities from setup. First 3 caps:`);
        devCaps.slice(0, 3).forEach((c) => {
          this._log(`  Setup cap: ${JSON.stringify(c)}`);
        });
        // Log writable caps specifically
        const writableCaps = devCaps.filter((c) => c.writable || c.access === 'rw' || c.settable);
        this._log(`Writable capabilities: ${JSON.stringify(writableCaps.map((c) => ({ id: c.capabilityId || c.id, name: c.name, type: c.type })))}`);
      }
    }
