...
const WATER_HEATER_CAP_IDS_BY_PRODUCT = {
  ...
  // productId 47 — the DHW tank of an Alfea Extensa Duo (modelId 1376, "Calypso
  // Split" in the reference table), reported over the Cozytouch/Magellan API
  // alongside its Overkiz-driven heat pump. Same idea as the AQUEO block above,
  // different product: none of the generic default IDs exist on it either.
  47: {
    HEATING_MODE: 87,
    TARGET_TEMP: 231,
    TARGET_TEMP_ALT: 22, // target_temperature_dhw, mirrors 231 on this tank
    ON_OFF: 86, // domestic_hot_water switch
    BOOST: 165,
    ECO: null,
    CURRENT_TEMP: 111, // dhw_temperature
    // Away mode here (226) is a start/stop timestamp pair, not a boolean
    // switch — writing '1'/'0' to it would be wrong, so it stays unmapped.
    AWAY_MODE: null,
    MIN_TEMP: 105301,
    MAX_TEMP: 105300,
  },
...
/**
 * Same per-product story as WATER_HEATER_CAP_IDS_BY_PRODUCT.
 *
 * These are not air conditioners. They are the heating *zones* of an Alféa
 * Excellia / Extensa heat pump — ROOM_0, ROOM_1, … — which land on this driver
 * because modelIds 557/558 sit in the range MODEL_TYPES calls AC. They answer
 * on the same block as a towel rack, not on the AC one: none of 1, 2, 4, 8, 9
 * exist on them, and **capability 7 carries the mode** (0=off, 4=heat) rather
 * than a temperature — read as CURRENT_TEMP it reported the mode enum as °C, so
 * a room at 21.97 °C showed up as 4 °C (ROOM_0), and a zone in heat mode showed
 * a flat 4 °C (ROOM_1).
 *
 * The room temperature is z-indexed per zone: 117 on ROOM_0, 118 on ROOM_1
 * (`thermostat_temperature_z1` / `_z2` upstream). It is reported only when a
 * room probe is actually installed — a weather-compensated installation with no
 * probe sends neither, which reads as an empty tile rather than a wrong value.
 *
 * Fan and swing are left on their default IDs: the product has neither, so the
 * reads return nothing and a write is refused with the usual message.
 *
 * ROOM_2…ROOM_4 (modelIds 559/560/561) are the same kind of endpoint and will
 * need the same treatment, but their productIds have not been observed yet, so
 * guessing a key here would only move the bug.
 */
const CLIMATE_CAP_IDS_BY_PRODUCT = {
  // ROOM_0, modelId 557
  26: {
    HVAC_MODE: 7,
    TARGET_TEMP_HEAT: 40,
    TARGET_TEMP_COOL: 177,
    CURRENT_TEMP: 117,
  },
  // ROOM_1, modelId 558 — same block, zone 2's temperature
  27: {
    HVAC_MODE: 7,
    TARGET_TEMP_HEAT: 40,
    TARGET_TEMP_COOL: 177,
...
/**
 * Capability IDs for a Magellan climate device, specialized for its product.
 * @param {number|string} [productId] - `productId` from the device store
 */
function climateCapIds(productId) {
  const override = CLIMATE_CAP_IDS_BY_PRODUCT[productId];
  return override ? { ...CLIMATE_CAP_IDS, ...override } : { ...CLIMATE_CAP_IDS };
}
