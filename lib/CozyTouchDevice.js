...
class CozyTouchDevice extends Homey.Device {
  ...
  _buildHandlerContext(store, data) {
    ...
    const ctx = {
      ...
      // Handlers may call this every poll; the wrapper skips no-op updates.
      setCapabilityOptions: (name, opts) =>
        this._safeSetCapabilityOptions(name, opts),
  ...
  async _poll() {
    try {
      ...
      this.error(`Poll failed (#${this._pollFailCount}):`, err.message);
  ...
  async _handlePollError(err) {
    ...
    this._markUnavailable(this.homey.__('errors.connection_failed'));
  }

  /**
   * Only call Homey when the device was actually unavailable. Repeating
   * setAvailable() on every successful poll can make the Flow editor briefly
   * rebuild cards ("Carte indisponible" flash) even though nothing failed.
   */
  _markAvailable() {
    if (typeof this.getAvailable === 'function' && this.getAvailable()) {
      return;
    }
  ...
  /**
   * Count real unavailability marks so a short UI flash can be distinguished
   * from an API/poll failure (the latter stays until the next OK poll).
   */
  _markUnavailable(message) {
    this._setUnavailableCount = (this._setUnavailableCount || 0) + 1;
    this.error(`setUnavailable (#${this._setUnavailableCount}): ${message}`);
    this.setUnavailable(message).catch(this.error);
  }

  /**
   * Homey documents setCapabilityOptions as expensive. Calling it every sync
   * with the same min/max/step forces a capability rebuild and is a likely
   * cause of brief "Carte indisponible" flashes in the Flow editor. Skip when
   * nothing changed — also cheaper on each poll.
   */
  _safeSetCapabilityOptions(name, opts) {
    if (!opts || !this.hasCapability(name)) {
      return;
    }
    let current = {};
    try {
    ...
    if (this._capabilityOptionsEqual(current, opts)) {
      return;
    }
  ...
  /**
   * Compare only keys present in `next` (what we are about to write). Homey
   * may return extra option fields we do not manage.
   */
  _capabilityOptionsEqual(current, next) {
    ...
    this.setCapabilityValue(name, value)
      .then(() => {
        ...
        if (previous !== null && previous !== undefined && previous !== value) {
          // Only on change, so a diagnostic report shows what the API actually
          // reported without spamming a line on every poll (see issue #9: the
          // one-time capability dump never covers what happens after it).
          this.log(`[${name}] API reports: ${previous} -> ${value}`);
