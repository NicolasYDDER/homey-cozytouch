...
class OverkizAPI {
  ...
  constructor({ username, password, log }) {
    ...
    this._loggedIn = false;  // True only once step 3 (Overkiz /login) succeeds
    this._devices = [];
    this._setup = null;
  ...
  async authenticate() {
    ...
    // Step 3: Login to Overkiz with JWT (form-encoded, captures session cookies)
    const loginBody = new URLSearchParams({ jwt: this._jwt }).toString();
    const loginResponse = await this._requestWithCookies('POST', OVERKIZ_API, '/login', loginBody, {
      'Content-Type': 'application/x-www-form-urlencoded',
    });
