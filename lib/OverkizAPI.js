...
class OverkizAPI {
  ...
  async authenticate() {
    ...
    const loginResponse = await this._request('POST', OVERKIZ_API, '/login', JSON.stringify({ jwt: this._jwt }), {
      'Content-Type': 'application/json',
    });

    if (loginResponse && loginResponse.success === false) {
      throw new Error(`Overkiz auth step 3 failed: ${loginResponse.error || 'login rejected'}`);
    }
  ...
  async _overkizGet(path) {
    return this._request('GET', OVERKIZ_API, path, null, {
  ...
  async _overkizPost(path, data) {
    return this._request('POST', OVERKIZ_API, path, data ? JSON.stringify(data) : null, {
      Authorization: `Bearer ${this._jwt}`,
      'Content-Type': 'application/json',
    });
  }
  ...
  _request(method, baseUrl, path, body, headers) {
    return new Promise((resolve, reject) => {
      // Concatenate base + path to preserve the base URL path component
      ...
      const options = {
        ...
        method,
        headers: headers || {},
        // Overkiz server sends responses with bare \n instead of \r\n
