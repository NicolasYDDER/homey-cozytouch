...
│   │   └── assets/                     # icon.svg + store images (small/large/xlarge)
│   │
│   ├── water_heater/
...
│   │   ├── driver.js
│   │   ├── handlers/
│   │   └── assets/                     # Pass module illustration + matching icon
│   │
│   ├── zone_control/
│   │   ├── device.js
│   │   ├── driver.js
│   │   ├── constants.js
│   │   ├── handlers/
│   │   └── assets/                     # Répartiteur + remotes illustration + icon
│   │
│   └── heat_pump/
...
│
├── tests/
│   └── driver-icons.test.js            # Guideline 1.6 checks on every driver icon.svg
│
└── tools/
    └── generate-driver-images.py       # Redraws the driver store images (not shipped)
```

Driver **store images** (guideline 1.4) are flat product illustrations on white, one set per driver, generated by `python3 tools/generate-driver-images.py`. Driver **icons** (guideline 1.6) are separate `currentColor` SVGs that mirror those devices; `tests/driver-icons.test.js` guards the Homey rules.
