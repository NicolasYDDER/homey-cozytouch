...
class HeaterDevice extends CozyTouchDevice {
  ...
  _updateFromOverkiz(states) {
    ...
    const minTemp = this._getStateValue(states, STATES.MIN_HEATING_TEMP);
    const maxTemp = this._getStateValue(states, STATES.MAX_HEATING_TEMP);
    if (minTemp !== null && maxTemp !== null) {
      this.setCapabilityOptions('target_temperature', {
        min: parseFloat(minTemp), max: parseFloat(maxTemp),
      }).catch(this.error);
