...
### Added
- **Atlantic Pass APC heat pumps** — new driver `heat_pump` for the air/water range (Alféa Extensa, Alféa Excellia, Hydrapac and the other units speaking the `io:PassAPC*` protocol). Reported for an **Alféa Extensa Duo A.I. 5 R32** (Magellan `modelId` 212): the heat pump was discovered but no driver claimed its endpoints, so pairing ended on "no compatible device found". Pairing now adds the **main unit** (`cozytouch_hvac_mode`, on/off, outside temperature from the stack's outside probe) and **one device per heating circuit** (setpoint 16–30 °C, room temperature from the circuit's own probe, `cozytouch_heating_mode` off/manual/prog, on/off).
- **The hot water tank of a Duo** is handled by the existing `water_heater` driver through a new `AtlanticPassAPCDHWComponent` handler (mode, setpoint, boost, absence), so a Duo pairs as heat pump + circuits + tank.
- New Flow trigger **HVAC mode changed** for `climate` and `heat_pump` main units — `cozytouch_hvac_mode` had a trigger for Zone Control only. `Set heating mode`, `Heating mode is…`, `Heating mode changed` and `Set HVAC mode` now also list heat pump devices.

### Changed
- **Commands are read from the device instead of guessed per model.** Pairing snapshots each Overkiz endpoint's `definition.commands` into the device store (`lib/helpers/overkiz-commands.js`), and every Pass APC control picks the first name its own endpoint advertises — one circuit takes `setHeatingTargetTemperature` where another only has `setDerogatedTargetTemperature`, and a heating-only unit has no `setHeatingCoolingAutoSwitch` at all. A missing command fails with a message naming it rather than sending something the heat pump refuses. Devices paired before this fall back to the usual name.
- **Cooling support is detected, not assumed**: cool/dry/auto modes and the `thermostat_mode` mirror only appear when the stack advertises a cooling command or a reversible circuit, so an Extensa is not offered modes it cannot do.
- **Endpoints are linked by component name, not by index.** One physical heat pump is several `io://gateway/deviceId#index` endpoints, and the indexes differ per product (the reported Extensa Duo has its main unit on `#1`, DHW on `#2`, outside probe on `#3` and a floor circuit on `#8`). A circuit finds its main unit and its own probe by name on the same stack (`findStackSibling`), so a two-circuit product does not read one probe twice.
- `heater`, `climate` and `towel_rack` now exclude Pass APC heat pump endpoints: they share `uiClass` `HeatingSystem` / `HeatPump` with devices those drivers do handle, and pairing an Alféa circuit there gave a device whose commands the heat pump refused.
- Zone Control protocol constants renamed to `PASS_APC_*` (both families speak the same protocol), with the `ZONE_CONTROL_*` names kept as aliases. `drivers/zone_control` runtime code is untouched.

### Fixed
- **A pairing failure now names every device found, in the app log.** The alert stays capped at 8 devices to remain readable, but the log line no longer ends in `+5`: on the report this release is based on, the endpoint that identified the product was inside that hidden remainder.
- **The Overkiz discovery log now includes `controllableName` and `widget`.** It only had `uiClass`, which cannot tell an Alféa circuit from a plain electric radiator — the component names had to be read off the pairing screen instead of the log.

### Note
- No hardware was available for this range, so nothing is guessed from values: roles come from component names, commands from what each endpoint advertises, and state reads fall back through the documented `core:*` setpoint states. `tests/heat-pump.test.js` covers the detection and linking rules against the reported stack.
- Magellan `modelId` 212 is deliberately **not** added to `MODEL_TYPES`: the same heat pump is also announced on Cozytouch/Magellan, and mapping it there would offer a second tile for one appliance on a protocol whose capability IDs for this product are unknown. The Overkiz endpoints are the supported ones.

## [1.3.7] - 2026-09-08

App Store certification round 1: everything here answers a point from the Homey review of 1.3.6.
**Never published on its own** — these changes reached users as part of 1.4.0.

### Changed
- **App description is a tagline, not a feature sentence** — guideline [1.2](https://apps.developer.homey.app/app-store/guidelines#id-1.2.-description) rejects "Control your … via Homey".
- **Driver images show the device on white**, one image per driver, instead of the stock photos of furnished rooms that guideline [1.4](https://apps.developer.homey.app/app-store/guidelines#id-1.4.-images) rejected. They are flat illustrations generated by [`tools/generate-driver-images.py`](tools/generate-driver-images.py) (4× oversampled, exported at 75/500/1000 px), so a driver image can be regenerated or restyled without hunting for source files. The script is excluded from the app bundle.
